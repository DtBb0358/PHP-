...
$city = mb_strimwidth($city, 0, 20);
$city = $city ?: '未知';

// 限制颜色值在合法范围内，使用“green”作为默认值
if (!in_array($color, ['blue', 'yellow', 'green', 'red','purple'])) {
    $color = 'green';
}
...
if ($id) {
    ...
    $sql = 'INSERT INTO `wish` (`name`,`color`,`content`,`password`,`city`,`time`) VALUES (?,?,?,?,?,?)';
    if (!$stmt = mysqli_prepare($link, $sql)) {
        exit("SQL[$sql]预处理失败：" . mysqli_error($link));
    }
    mysqli_stmt_bind_param($stmt, 'sssssi', $name, $color, $content, $password, $city, $time);
}
